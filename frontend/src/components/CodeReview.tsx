...
`;
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      if (result.error || !result.readability_score) {
        setError('Failed to parse AI response.');
        console.warn('RAW GPT RESPONSE:', result.raw);
      } else {
        setScores(result);
      }
    } catch (err) {
  ...
  const handleRepoReview = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/review_repo', {
        method: 'POST',
      ...
      console.log("Repo review response:", data); // ✅ Add this
